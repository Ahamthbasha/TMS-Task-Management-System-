// src/repositories/genericRepo/genericRepo.ts
import { Document, Model, QueryFilter, UpdateQuery, QueryOptions } from 'mongoose';
...
export class GenericRepository<T extends Document> implements IGenericRepository<T> {
  ...
  async findOne(filter: QueryFilter<T>, projection?: string): Promise<T | null> {
